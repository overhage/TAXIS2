...
// ===== Configs =====
// >>> BEGIN MODIFIED: increase default run budget to yield before Netlify's 15-min cap
const MAX_RUN_MS = Number(process.env.MAX_RUN_MS || 13.5 * 60 * 1000)
// <<< END MODIFIED
...
export default async (req) => {
  try {
    ...
    const __beat = async (processedCount) => {
      const now = Date.now()
      if (now - __lastBeat >= 20_000) {
        await prisma.job.update({ where: { id: jobId }, data: { rowsProcessed: processedCount, lastHeartbeat: new Date() } })
        __lastBeat = now
      }
    }
    // <<< END ADDED
